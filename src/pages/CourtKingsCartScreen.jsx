...
import CourtKingsCartItemComponent from '../components/CourtKingsCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'CourtKingsCartSuccessScreen'
      : 'CourtKingsHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <CourtKingsCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
